...
export class SymbolIndex {
	...
	async indexFiles(files: ProjectFile[]): Promise<void> {
		...
		for (const file of files) {
			if (!file.language || !this.analyzer.supports(file.language)) continue
			let source: string
			try { source = await fs.readFile(file.absolutePath, "utf8") } catch { continue }
			...
			for (const symbol of analysis.symbols) {
				...
				this.symbolsById.set(id, { ...symbol, id, filePath: file.relativePath })
			}
		}
	}

	getAll(): SymbolRecord[] { return [...this.symbolsById.values()] }
	getAnalyses(): Map<string, StructuralAnalysis> { return new Map(this.analysesByFile) }
	...
	getByFile(filePath: string): SymbolRecord[] { return this.find({ filePath, limit: Number.MAX_SAFE_INTEGER }) }
	getAnalysis(filePath: string): StructuralAnalysis | undefined { return this.analysesByFile.get(filePath) }
	clear(): void { this.symbolsById.clear(); this.analysesByFile.clear() }

	private symbolId(filePath: string, symbol: StructuralSymbol): string {
		return crypto.createHash("sha1").update(`${filePath}:${symbol.kind}:${symbol.parentName ?? ""}:${symbol.name}:${symbol.location.start}`).digest("hex")
	}
}
