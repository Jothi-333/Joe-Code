	...
	analyze(
		...
		const addSymbol = (
			...
			symbols.push({
				...
				async: modifiers?.some((modifier) => modifier.kind === ts.SyntaxKind.AsyncKeyword),
				static: modifiers?.some((modifier) => modifier.kind === ts.SyntaxKind.StaticKeyword),
			})
		}

		const visit = (node: ts.Node, parentName?: string): void => {
			if (ts.isClassDeclaration(node) && node.name) {
				addSymbol(node, node.name.text, "class", parentName)
			...
			if (ts.isFunctionDeclaration(node) && node.name) {
				addSymbol(node, node.name.text, "function", parentName)
			}

			if (ts.isInterfaceDeclaration(node)) {
				addSymbol(node, node.name.text, "interface", parentName)
			}

			if (ts.isTypeAliasDeclaration(node)) {
				addSymbol(node, node.name.text, "type", parentName)
			}

			if (ts.isEnumDeclaration(node)) {
				addSymbol(node, node.name.text, "enum", parentName)
			}

			if (ts.isVariableStatement(node)) {
				for (const declaration of node.declarationList.declarations) {
					const names = this.bindingNames(declaration.name)
					for (const name of names) {
						const initializer = declaration.initializer
						const kind: StructuralSymbolKind = initializer &&
							(ts.isArrowFunction(initializer) || ts.isFunctionExpression(initializer))
							? "function"
							: "variable"
						addSymbol(node, name, kind, parentName)
					}
				}
			}

			if (ts.isImportDeclaration(node)) {
				imports.push(this.parseImport(sourceFile, node))
			}

			if (ts.isExportDeclaration(node)) {
				this.parseExportDeclaration(sourceFile, node, exports)
			}
			...
			if (node !== sourceFile) {
				for (const child of node.getChildren(sourceFile)) {
					visit(child, this.nextParentName(node, parentName))
				}
			} else {
	...
	private bindingNames(name: ts.BindingName): string[] {
		...
		for (const element of name.elements) {
			if (ts.isBindingElement(element)) {
				names.push(...this.bindingNames(element.name))
			}
		}
		return names
	}

	private nextParentName(node: ts.Node, parentName?: string): string | undefined {
